...
def build_passkey_text(target_tokens: int, position_ratio: float) -> str:
    filler = "The archive contains neutral background text for retrieval evaluation. "
    needle = "The pass key is 482917."
    question = "What is the pass key?"
    approx_words = max(target_tokens, 128)
    parts = [filler] * approx_words
    ...
    for length in lengths:
        for pos in [0.1, 0.3, 0.5, 0.7, 0.9]:
            ...
            for _ in range(samples):
                text = build_passkey_text(length, pos)
                ids = tokenizer.encode(text).ids[: config.max_seq_len]
                input_ids = torch.tensor([ids], dtype=torch.long, device=device)
                out = model.generate(input_ids, max_new_tokens=16)[0].tolist()
                decoded = tokenizer.decode(out)
                correct += int("482917" in decoded)
            rows.append(
                {
                    "model": config.name,
                    "length": length,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Passkey Retrieval 评测")
    ...
    parser.add_argument("--output", default="outputs/artifacts/passkey_results.csv")
