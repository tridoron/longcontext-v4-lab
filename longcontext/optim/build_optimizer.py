...
def _is_no_decay_name(name: str, param: torch.nn.Parameter) -> bool:
    if param.ndim <= 1:
        return True
    markers = ("norm", "bias", "a_raw", "b_raw", "c_raw", "a_final_raw", "beta_raw")
...
def split_parameter_groups(
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        if _is_no_decay_name(name, param):
            ...
            adamw_no_decay.append(param)
            adamw_names.append(f"no_decay\t{name}")
    return muon_params, adamw_decay, adamw_no_decay, muon_names, adamw_names
...
def build_optimizer(
    ...
    muon_params, adamw_decay, adamw_no_decay, muon_names, adamw_names = split_parameter_groups(
        model, use_muon=use_muon
    )
    ...
    if adamw_decay:
        adamw_groups.append(
            {"params": adamw_decay, "weight_decay": float(adamw_cfg.get("weight_decay", 0.1))}
        )
    if adamw_no_decay:
        adamw_groups.append(
            {"params": adamw_no_decay, "weight_decay": float(adamw_cfg.get("no_decay_weight_decay", 0.0))}
        )
    if adamw_groups:
        optimizers.append(
            torch.optim.AdamW(
